export const translations = {
  id: {
    // Brand & Slogan
    brandName: 'SportBook',
    slogan: 'Venue Booking System',
    heroHeadline1: 'Find Your Court.',
    heroHeadline2: 'Book Your Game.',
    heroSubheadline: 'Temukan lapangan badminton, padel, pingpong, futsal, minisoccer, dan tenis. Pilih waktu yang tersedia dan booking instan tanpa ribet.',
    quickSearchTitle: 'Cari Lapangan Instan',
    
    // Navbar
    navOutlets: 'Manage Outlet',
    navSports: 'Jenis Olahraga',
    navCourts: 'Daftar Lapangan',
    navAvailability: 'Availability',
    navFacilities: 'Fasilitas',
    navWhyUs: 'Mengapa Kami',
    btnCheckBooking: 'CEK BOOKING',
    btnDashboard: 'Dashboard',
    btnBookNow: 'Booking Sekarang',
    btnCustomerView: 'Lihat Tampilan Customer',

    // Outlets Section
    outletsTag: 'Cabang & Lokasi Venue',
    outletsHeading: 'Pilih Lokasi Outlet Olahraga',
    outletsSub: 'Pilih cabang venue terlebih dahulu untuk menampilkan lapangan dan jadwal ketersediaan yang sesuai.',
    allOutlets: 'Semua Outlet (Tampilkan Semua)',
    allOutletsShort: 'Semua Outlet',
    selectOutlet: 'Pilih Outlet',
    outletLabel: 'Lokasi Outlet',
    outletFilter: 'Filter Berdasarkan Outlet',
    courtsInOutlet: 'Lapangan',
    viewCourtsInOutlet: 'Lihat Lapangan',
    outletMgmtTitle: 'Pengelolaan Outlet (Outlet Management)',
    outletMgmtSub: 'Kelola cabang venue, alamat lengkap, nomor kontak, foto, dan status aktif',
    addNewOutletBtn: 'TAMBAH OUTLET BARU',
    editOutletTitle: 'Edit Data Outlet',
    addNewOutletTitle: 'Tambah Outlet Baru',
    outletNameLabel: 'Nama Outlet *',
    outletAddressLabel: 'Alamat Lengkap *',
    outletPhoneLabel: 'Nomor Telepon / WhatsApp',
    outletDescLabel: 'Deskripsi Singkat Outlet',
    outletImageUrlLabel: 'URL Foto Outlet',
    outletStatusLabel: 'Status Outlet',
    saveOutletBtn: 'Simpan Outlet',
    loadingOutlets: 'Memuat data outlet...',
    unassignedOutlet: 'Tanpa Outlet (Unassigned)',
    assignOutletLabel: 'Penugasan ke Outlet *',
    tableOutlet: 'Outlet Venue',

    // Badges & Features
    badgeNoLogin: 'BOOKING REAL-TIME TANPA LOGIN',
    featAntiDouble: 'Anti Double Booking',
    featProStandard: 'Standar Pro & AC',
    featNoAccount: 'Tanpa Akun / Login',

    // Search Widget
    labelSport: 'Olahraga',
    allSports: 'Semua Olahraga (Badminton, Padel, dll)',
    labelDate: 'Tanggal',
    labelStartTime: 'Jam Mulai',
    btnSearchAvailable: 'CARI LAPANGAN AVAILABLE',

    // Sports Categories
    catTitle: 'Kategori Olahraga',
    catHeading: 'Pilih Cabang Olahraga Favoritmu',
    catSub: 'Klik olahraga di bawah untuk memfilter daftar lapangan',
    allCourts: 'Semua Lapangan',
    showAll: 'Tampilkan Semua',

    // Popular Courts
    courtsBadge: 'DAFTAR LAPANGAN POPULER',
    courtsHeading: 'Pilih Lapangan Olahraga',
    courtsSub: 'Lihat ketersediaan jam secara real-time dan langsung kunci slot bermainmu.',
    perHour: '/ jam',
    btnBookSelectDate: 'PILIH TANGGAL & BOOKING',
    noCourtsMatch: 'Tidak ada lapangan yang cocok dengan filter saat ini.',
    availabilityFor: 'Ketersediaan',

    // Availability UI & Legend
    availTag: 'Time Slot Grid',
    availHeading: 'Cek Ketersediaan Jam Lapangan',
    availDateLabel: 'Tanggal yang dipilih:',
    availLegendAvailable: '🟢 Tersedia (Available)',
    availLegendSelected: '🔵 Dipilih (Selected)',
    availLegendBooked: '⚪ Terisi (Booked)',
    tableCourtHeader: 'Lapangan / Waktu',
    statusAvailable: 'Available',
    statusBooked: 'Booked',

    // Why Choose Us
    whyUsTag: 'Keunggulan SportBook',
    whyUsHeading: 'Mengapa Memilih Booking di Venue Kami?',
    whyUsSub: 'Pengalaman reservasi yang cepat, transparan, dan terpercaya untuk semua pecinta olahraga.',
    feat1Title: 'Validasi Anti Double-Booking',
    feat1Desc: 'Sistem otomatis backend & database mengunci slot jam agar tidak bentrok antar pemesan.',
    feat2Title: 'Booking Instan Tanpa Login',
    feat2Desc: 'Langsung pilih waktu, isi data diri singkat, dan dapatkan konfirmasi booking dalam hitungan detik.',
    feat3Title: 'Fasilitas Standar Turnamen',
    feat3Desc: 'Pencahayaan LED profesional, lantai anti-slip, AC dingin, serta ruang ganti & shower bersih.',
    feat4Title: 'Jam Operasional Panjang',
    feat4Desc: 'Buka setiap hari mulai pukul 08:00 sampai 23:00 WIB untuk memenuhi jadwal olahraga favoritmu.',

    // Footer & WA
    footerDesc: 'Find Your Court. Book Your Game. Platform reservasi lapangan olahraga paling modern, cepat, dan tanpa ribet.',
    footerCustomerService: 'Layanan Customer',
    footerContactUs: 'Hubungi Kami',
    chatWAAdmin: 'Chat WA Admin',

    // Booking Modal
    modalTitleForm: 'Form Booking Lapangan',
    modalTitleSuccess: 'Booking Berhasil Confirmed!',
    courtPriceLabel: 'Harga / jam',
    labelDuration: 'Durasi',
    duration1: '1 Jam',
    duration2: '2 Jam',
    duration3: '3 Jam',
    duration4: '4 Jam',
    custDataTitle: 'Data Penyewaan (Tanpa Login)',
    fullName: 'Nama Lengkap *',
    phone: 'No. WhatsApp / HP *',
    email: 'Email',
    totalPriceLabel: 'Total Biaya Penyewaan:',
    btnConfirmBooking: 'KONFIRMASI BOOKING',
    bookingCodeLabel: 'Kode Booking:',
    saveCodeNotice: 'Simpan kode ini untuk melakukan check-in di lokasi venue.',
    paymentStatusLabel: 'Status Pembayaran:',
    payTransferNote: '(Bayar di Tempat / Transfer)',
    bankInstructionsTitle: 'Petunjuk Pembayaran Transfer Bank:',
    btnCloseReturn: 'TUTUP & KEMBALI KE BERANDA',
    multiBookingTipTitle: 'Tips Booking Lebih Dari 1 Lapangan:',
    multiBookingTipDesc: 'Gunakan Nama Lengkap & No. WhatsApp yang sama persis. Selama tagihan sebelumnya masih UNPAID, booking baru akan otomatis digabungkan dalam 1 Kode Booking & 1 Tagihan!',
    mergedBookingNotice: 'Booking baru ini telah otomatis digabungkan ke Kode Booking',
    rentedCourtsList: 'Daftar Lapangan Disewa:',
    qrisPaymentTitle: 'Pembayaran via QRIS',
    qrisScanDesc: 'Scan QRIS menggunakan GoPay, OVO, Dana, ShopeePay, BCA Mobile, dll.',
    processing: 'Memproses...',
    paymentDeadlineTitle: 'Informasi Batas Waktu Pembayaran',
    paymentDeadlineTime: 'Batas Waktu Telat Bayar',
    orderCreatedTime: 'Waktu Booking Dibuat',
    latePaymentNotice: 'Telat Bayar (Lewat Batas Waktu)',
    paymentLimitHoursLabel: 'Batas Waktu Pembayaran (Jam)',
    paymentLimitHoursSub: 'Atur batas jam telat bayar setelah booking dibuat (default: 1 jam)',

    // Check Booking Modal
    checkModalTitle: 'Cek Status Booking & Konfirmasi Bayar',
    checkInputPlaceholder: 'Contoh: SB-1001 atau 081234567890',
    btnCheck: 'CEK',
    searching: 'Mencari...',
    uploadProofTitle: 'Upload Bukti Transfer Pembayaran',
    uploadProofSub: 'Unggah foto/struk transfer agar staff venue dapat memverifikasi pembayaran Anda.',
    btnUploadProof: 'UNGGAH BUKTI TRANSFER',
    uploadingProofMsg: 'Mengunggah...',
    proofReadyMsg: 'Bukti Transfer Ready',
    proofReadySub: 'Klik tombol di bawah untuk unggah',
    orWhatsApp: 'ATAU BUKTI VIA WHATSAPP',
    btnSendWA: 'KIRIM BUKTI PEMBAYARAN KE WHATSAPP ADMIN',
    bankPaymentDetails: 'Rekening Pembayaran Venue',
    scanQris: 'Scan QRIS',

    // Admin Login Modal
    staffLoginTitle: 'Portal Login Staff & Operator Venue',
    usernameLabel: 'Username Staff *',
    passwordLabel: 'Password *',
    usernamePlaceholder: 'Masukkan username',
    passwordPlaceholder: 'Masukkan password',
    btnEnterDashboard: 'MASUK DASHBOARD',
    loggingIn: 'Memproses Login...',
    demoAccountsTitle: 'Akun Akses Berdasarkan Level Role:',
    returnToHome: 'Kembali ke Landing Page Utama',

    // Manual Staff Booking Modal
    manualBookingTitle: 'Booking Manual (Input Staff / On-site)',
    manualBookingSuccessTitle: 'Booking Manual Berhasil Dibuat!',
    manualBookingSuccessSub: 'Jadwal telah diperbarui di sistem dan kalender visual.',
    selectCourtLabel: 'Pilih Lapangan *',
    custDataManualTitle: 'Data Customer (Walk-in / Telepon)',
    custNameLabel: 'Nama Customer *',
    btnClose: 'TUTUP',
    btnCancel: 'Batal',
    btnSaveManualBooking: 'SIMPAN BOOKING MANUAL',
    savingManualBooking: 'Menyimpan...',

    // Management Dashboard (Admin)
    adminTitle: 'Management Console',
    btnManualStaff: 'BOOKING MANUAL STAFF',
    navDashboard: 'Dashboard',
    navBookings: 'Bookings',
    navCalendar: 'Calendar',
    navCourts: 'Courts',
    navSports: 'Sports',
    navCustomers: 'Customers',
    navPayments: 'Payments',
    navReports: 'Reports',
    navUsers: 'Users',
    navSettings: 'Settings',
    btnViewMainWeb: 'Lihat Website Main',
    statTodayRev: 'Revenue Hari Ini',
    statTodayBookings: 'Booking Hari Ini',
    statOccupiedCourts: 'Lapangan Terpakai',
    statOccupancyRate: 'Occupancy Rate',
    tableBookingCode: 'Kode Booking',
    tableCustomer: 'Customer',
    tableCourtSport: 'Lapangan & Sport',
    tableDateTime: 'Tanggal & Jam',
    tableTotalPrice: 'Total Biaya',
    tableProof: 'Bukti Transfer',
    tableStatus: 'Status Booking',
    tableActions: 'Kelola Status / Aksi Staff',
    btnDeletePermanent: 'Hapus Permanen',
    btnSetPaid: 'Set Paid',
    btnViewProof: 'Lihat Bukti',

    // Overview Tab
    loadingDashboard: 'Memuat data dashboard...',
    upcomingBookingsTitle: 'Upcoming Bookings',
    upcomingBookingsSub: 'Daftar reservasi mendatang',
    statBySport: 'Statistik Per Olahraga',
    sessions: 'Sesi',
    recordedInSystem: 'Tercatat di sistem',
    courtsAvailable: 'Lapangan Tersedia',
    noUpcomingBookings: 'Belum ada booking mendatang',

    // Bookings & Payments Tab
    searchBookingsPlaceholder: 'Cari kode booking, nama penyewa, atau hp...',
    allBookingStatus: 'Semua Status Booking',
    exportExcel: 'EXPORT EXCEL',
    noBookingsFound: 'Tidak ada data booking ditemukan.',
    showingData: 'Menampilkan',
    to: 'sampai',
    of: 'dari',
    bookingsCount: 'data booking',
    exportModalTitle: 'Export Data ke Excel',
    exportDateStart: 'Tanggal Awal *',
    exportDateEnd: 'Tanggal Akhir *',
    downloadExcelBtn: 'DOWNLOAD FILE EXCEL',
    transferProofModalTitle: 'Bukti Transfer Pembayaran',
    verifySetPaidBtn: 'VERIFIKASI & SET PAID',
    deleteProofBtn: 'Hapus Bukti',
    btnRefreshBookings: 'REFRESH & CEK TELAT BAYAR',
    autoCancelledMsg: 'booking telat bayar telah otomatis diubah statusnya menjadi Cancelled.',
    noOverdueMsg: 'Tidak ada booking telat bayar yang perlu dibatalkan.',
    btnPrintReceipt: 'Cetak Nota',
    btnPrintReceiptShort: 'Print Nota',
    receiptTitle: 'NOTA PEMBAYARAN',
    receiptSub: 'Bukti Pembayaran & Sewa Lapangan Resmi',
    receiptNo: 'No. Nota / Booking',
    receiptDate: 'Tanggal Cetak',
    receiptCustomerInfo: 'Informasi Pelanggan',
    receiptBookingDetails: 'Rincian Sewa Lapangan',
    receiptItem: 'Item / Lapangan',
    receiptSchedule: 'Jadwal & Durasi',
    receiptPrice: 'Biaya',
    receiptSubtotal: 'Subtotal',
    receiptTotalPayment: 'TOTAL PEMBAYARAN',
    receiptPaymentMethod: 'Metode Pembayaran',
    receiptPaymentStatus: 'Status Pembayaran',
    receiptBookingStatus: 'Status Booking',
    receiptStatusPaid: 'LUNAS',
    receiptStatusFinished: 'SELESAI',
    receiptThankYou: 'Terima Kasih Telah Berolahraga Bersama Kami!',
    receiptContactNotice: 'Harap simpan nota ini sebagai bukti transaksi resmi.',
    paymentsFinishedNotice: 'Menampilkan daftar transaksi pembayaran booking dengan status Finished (Selesai).',
    eposPrinterMode: 'Printer EPOS Thermal (58mm / 80mm POS)',
    standardPrinterMode: 'Dokumen Standar (A4)',
    btnPrintEpos: 'PRINT KE PRINTER EPOS',
    eposLayoutTitle: 'Format Struk Thermal EPOS',

    // Calendar Tab
    visualCalendarTitle: 'Visual Court Schedule Grid',
    visualCalendarSub: 'Matriks ketersediaan jam per lapangan. Klik slot terisi untuk kelola status / hapus booking.',
    hourHeader: 'Jam',
    loadingCalendar: 'Memuat jadwal kalender...',
    manageBookingStatusTitle: 'Kelola Status Booking Staff',
    currentStatusLabel: 'Status Saat Ini:',
    finishBtn: 'Selesai',

    // Courts Tab
    courtMgmtTitle: 'Pengelolaan Lapangan (Court Management)',
    courtMgmtSub: 'Kelola daftar lapangan, harga per jam, foto, dan status aktif',
    addNewCourtBtn: 'TAMBAH LAPANGAN BARU',
    editCourtTitle: 'Edit Data Lapangan',
    addNewCourtTitle: 'Tambah Lapangan Baru',
    sportCategoryLabel: 'Cabang Olahraga *',
    courtNameLabel: 'Nama Lapangan *',
    pricePerHourLabel: 'Harga Per Jam (Rp) *',
    imageUrlLabel: 'URL Foto Lapangan',
    facilitiesCommaLabel: 'Fasilitas (Pisahkan dengan koma)',
    courtStatusLabel: 'Status Lapangan',
    activeStatus: 'Active (Siap Sewa)',
    maintenanceStatus: 'Maintenance (Perbaikan)',
    saveCourtBtn: 'Simpan Lapangan',
    loadingCourts: 'Memuat daftar lapangan...',

    // Sports Tab
    addSportHeading: 'Tambah Cabang Olahraga',
    sportNameLabel: 'Nama Olahraga *',
    shortDescLabel: 'Deskripsi Singkat',
    addSportBtn: '+ TAMBAH OLAHRAGA',
    sportsListHeading: 'Kategori Olahraga Terdaftar',

    // Customers Tab
    customerDirTitle: 'Data Pelanggan (Customer Directory)',
    customerDirSub: 'Riwayat penyewa, total booking, dan total pengeluaran',
    searchCustomerPlaceholder: 'Cari nama, no hp, atau email...',
    noCustomersFound: 'Tidak ada data customer ditemukan.',
    loadingCustomers: 'Memuat data penyewa...',
    totalBookingsLabel: 'Total Sesi Booking',
    totalSpentLabel: 'Total Spent',

    // Reports Tab
    reportsHeading: 'Laporan Pendapatan & Occupancy Venue',
    reportsSub: 'Ringkasan performa bisnis venue per lapangan & cabang olahraga',
    revenuePerCourt: 'Laporan Revenue Per Lapangan',
    occupancyMetrics: 'Metrik Occupancy Rate',
    todayUsage: 'Tingkat Penggunaan Hari Ini',
    totalActiveCourts: 'Total Lapangan Aktif:',
    occupiedCourtsCount: 'Lapangan Terpakai:',
    emptyCourtsCount: 'Lapangan Kosong:',

    // Settings Tab
    venuePaymentConfig: 'Venue Payment Configuration',
    paymentSettingsHeading: 'Pengaturan Metode Pembayaran',
    paymentSettingsSub: 'Atur informasi Nomor Rekening Bank dan Gambar QRIS venue untuk transaksi pembayaran customer.',
    bankInfoHeading: 'Informasi Rekening Bank Venue',
    bankInfoSub: 'Nomor rekening transfer yang akan ditampilkan pada tiket booking customer.',
    bankNameField: 'Nama Bank *',
    accountNumberField: 'Nomor Rekening *',
    accountHolderField: 'Atas Nama (Pemilik) *',
    waSupportHeading: 'Nomor WhatsApp Support / Chat Live',
    waSupportSub: 'Nomor WhatsApp ini digunakan untuk tombol melayang (Floating Widget) di Beranda Utama & konfirmasi pembayaran.',
    waAdminField: 'Nomor WhatsApp Admin (Kode Negara 62...) *',
    qrisInfoHeading: 'Informasi & Barcode QRIS',
    qrisInfoSub: 'Kelola gambar QRIS venue yang digunakan customer untuk pembayaran digital instant.',
    merchantNameField: 'Nama Merchant / Keterangan QRIS',
    activeQrisImage: 'Gambar QRIS Active',
    changeQrisBtn: 'Ganti Gambar QRIS',
    deleteQrisBtn: 'Hapus QRIS',
    noQrisTitle: 'Belum Ada Gambar QRIS',
    noQrisSub: 'Unggah file gambar kode QRIS venue Anda (format PNG, JPG, atau WebP).',
    uploadNewQrisBtn: 'Upload Gambar QRIS Baru',
    savePaymentSettingsBtn: 'SIMPAN PENGATURAN PEMBAYARAN',
    savingSettingsMsg: 'Menyimpan...',

    // Users Management Tab
    usersMgmtTitle: 'Manajemen Pengguna & Staff Cabang',
    usersMgmtSub: 'Kelola akun Super Admin dan Operator Outlet dengan hak akses multi-cabang terisolasi',
    exportUsersTitle: 'Export data staff ke file Excel (.csv)',
    addStaffBtn: 'Tambah Staff / User',
    totalUsersLabel: 'Total Pengguna',
    superAdminLabel: 'Super Admin',
    operatorOutletLabel: 'Operator Outlet',
    activeAccountsLabel: 'Akun Aktif',
    searchUsersPlaceholder: 'Cari nama staff / username...',
    allRoles: 'Semua Role',
    roleSuperAdmin: 'Super Admin',
    roleOperatorOutlet: 'Operator Outlet',
    allBranches: 'Semua Cabang',
    registeredStaff: 'Staff Terdaftar',
    colStaffName: 'Nama Staff & Username',
    colRole: 'Role / Tingkatan',
    colAssignedOutlet: 'Cabang Outlet Terassign',
    colStatus: 'Status',
    colCreatedAt: 'Tanggal Dibuat',
    colActions: 'Aksi',
    loadingUsers: 'Memuat data pengguna...',
    noUsersFound: 'Tidak ada pengguna yang sesuai dengan filter.',
    allBranchesFullAccess: 'Semua Cabang (Full Access)',
    unassignedRole: 'Belum Diassign',
    statusActive: 'Aktif',
    statusInactive: 'Nonaktif',
    editUserTooltip: 'Edit Pengguna',
    deleteUserTooltip: 'Hapus Pengguna',
    editStaffTitle: 'Edit Akun Staff',
    addStaffTitle: 'Tambah Akun Staff Baru',
    labelFullNameStaff: 'Nama Lengkap Staff:',
    phFullNameStaff: 'Contoh: Rian Anggara',
    labelUsernameLogin: 'Username Login:',
    phUsernameLogin: 'Contoh: staff_cilandak',
    helpUsername: 'Hanya huruf kecil, angka, dan underscore',
    labelNewPassword: 'Password Baru (Kosongkan jika tidak diganti):',
    labelAccountPassword: 'Password Akun:',
    phKeepPassword: '•••••••• (Tetap)',
    phMin6Chars: 'Minimal 6 karakter',
    labelRoleAccess: 'Role / Tingkatan Akses:',
    optOperator: 'Operator Outlet (Terisolasi per Cabang)',
    optAdmin: 'Super Admin (Akses Penuh Semua Cabang)',
    labelAssignOutlet: 'Assign ke Cabang Outlet:',
    optSelectOutlet: '-- Pilih Cabang Outlet --',
    helpOutletAssign: 'Staff ini hanya dapat mengelola data lapangan, jadwal, booking, dan pembayaran di cabang outlet yang dipilih.',
    labelAccountStatus: 'Status Akun:',
    optActive: 'Aktif',
    optInactive: 'Nonaktif (Blokir Login)',
    btnSaveChanges: 'Simpan Perubahan',
    btnCreateUser: 'Buat User',
    savingMsg: 'Menyimpan...',
    errPasswordRequired: 'Password wajib diisi untuk user baru',
    errOperatorOutletRequired: 'User dengan role Operator harus di-assign ke salah satu outlet',
    errSaveUser: 'Gagal menyimpan user',
    alertCannotDeleteSuperAdmin: 'Super Admin utama tidak dapat dihapus!',
    errDeleteUser: 'Gagal menghapus user',
    excelUserId: 'ID User',
    excelFullName: 'Nama Lengkap',
    excelUsername: 'Username',
    excelRole: 'Role / Tingkatan',
    excelOutletName: 'Cabang Outlet Ditugaskan',
    excelStatus: 'Status Akun',
    excelCreatedAt: 'Tanggal Dibuat'
  },
  en: {
    // Brand & Slogan
    brandName: 'SportBook',
    slogan: 'Venue Booking System',
    heroHeadline1: 'Find Your Court.',
    heroHeadline2: 'Book Your Game.',
    heroSubheadline: 'Discover badminton, padel, pingpong, futsal, minisoccer, and tennis courts. Select available slots and book instantly hassle-free.',
    quickSearchTitle: 'Instant Court Search',

    // Navbar
    navOutlets: 'Manage Outlets',
    navSports: 'Sports Categories',
    navCourts: 'Courts Directory',
    navAvailability: 'Availability',
    navFacilities: 'Facilities',
    navWhyUs: 'Why Choose Us',
    btnCheckBooking: 'CHECK BOOKING',
    btnDashboard: 'Dashboard',
    btnBookNow: 'Book Now',
    btnCustomerView: 'Customer View',

    // Outlets Section
    outletsTag: 'Branches & Venue Locations',
    outletsHeading: 'Select a Sports Venue Outlet',
    outletsSub: 'Select a venue branch location first to explore matching courts and real-time schedule availability.',
    allOutlets: 'All Outlets (Show All)',
    allOutletsShort: 'All Outlets',
    selectOutlet: 'Select Outlet',
    outletLabel: 'Outlet Location',
    outletFilter: 'Filter by Outlet',
    courtsInOutlet: 'Courts',
    viewCourtsInOutlet: 'View Courts',
    outletMgmtTitle: 'Outlet Management',
    outletMgmtSub: 'Manage venue branches, full addresses, contact numbers, photos, and active status',
    addNewOutletBtn: 'ADD NEW OUTLET',
    editOutletTitle: 'Edit Outlet Data',
    addNewOutletTitle: 'Add New Outlet',
    outletNameLabel: 'Outlet Name *',
    outletAddressLabel: 'Full Address *',
    outletPhoneLabel: 'Phone / WhatsApp Number',
    outletDescLabel: 'Short Outlet Description',
    outletImageUrlLabel: 'Outlet Photo URL',
    outletStatusLabel: 'Outlet Status',
    saveOutletBtn: 'Save Outlet',
    loadingOutlets: 'Loading outlets data...',
    unassignedOutlet: 'Unassigned Outlet',
    assignOutletLabel: 'Assign to Outlet *',
    tableOutlet: 'Venue Outlet',

    // Badges & Features
    badgeNoLogin: 'REAL-TIME BOOKING WITHOUT LOGIN',
    featAntiDouble: 'Anti Double Booking',
    featProStandard: 'Pro Standard & AC',
    featNoAccount: 'No Account Required',

    // Search Widget
    labelSport: 'Sport',
    allSports: 'All Sports (Badminton, Padel, etc.)',
    labelDate: 'Date',
    labelStartTime: 'Start Time',
    btnSearchAvailable: 'SEARCH AVAILABLE COURTS',

    // Sports Categories
    catTitle: 'Sport Categories',
    catHeading: 'Choose Your Favorite Sport',
    catSub: 'Click a sport below to filter the court directory',
    allCourts: 'All Courts',
    showAll: 'Show All',

    // Popular Courts
    courtsBadge: 'POPULAR COURTS DIRECTORY',
    courtsHeading: 'Select a Sports Court',
    courtsSub: 'View real-time slot availability and lock in your game time instantly.',
    perHour: '/ hour',
    btnBookSelectDate: 'SELECT DATE & BOOK',
    noCourtsMatch: 'No courts match the current filter.',
    availabilityFor: 'Availability',

    // Availability UI & Legend
    availTag: 'Time Slot Grid',
    availHeading: 'Check Court Slot Availability',
    availDateLabel: 'Selected Date:',
    availLegendAvailable: '🟢 Available',
    availLegendSelected: '🔵 Selected',
    availLegendBooked: '⚪ Booked',
    tableCourtHeader: 'Court / Time',
    statusAvailable: 'Available',
    statusBooked: 'Booked',

    // Why Choose Us
    whyUsTag: 'SportBook Advantages',
    whyUsHeading: 'Why Book at Our Venue?',
    whyUsSub: 'Fast, transparent, and reliable reservation experience for all sports lovers.',
    feat1Title: 'Anti Double-Booking Validation',
    feat1Desc: 'Automated backend & database system locks time slots to prevent conflicting reservations.',
    feat2Title: 'Instant Guest Booking',
    feat2Desc: 'Pick your time slot, fill brief details, and receive booking confirmation in seconds.',
    feat3Title: 'Tournament Grade Facilities',
    feat3Desc: 'Professional LED lighting, anti-slip flooring, cool AC, clean lockers & showers.',
    feat4Title: 'Extended Operating Hours',
    feat4Desc: 'Open daily from 08:00 to 23:00 WIB to fit your favorite playing schedule.',

    // Footer & WA
    footerDesc: 'Find Your Court. Book Your Game. The most modern, fast, and hassle-free sports court reservation platform.',
    footerCustomerService: 'Customer Service',
    footerContactUs: 'Contact Us',
    chatWAAdmin: 'Chat WA Admin',

    // Booking Modal
    modalTitleForm: 'Court Booking Form',
    modalTitleSuccess: 'Booking Confirmed Successfully!',
    courtPriceLabel: 'Rate / hour',
    labelDuration: 'Duration',
    duration1: '1 Hour',
    duration2: '2 Hours',
    duration3: '3 Hours',
    duration4: '4 Hours',
    custDataTitle: 'Customer Details (No Login)',
    fullName: 'Full Name *',
    phone: 'WhatsApp / Phone No *',
    email: 'Email Address',
    totalPriceLabel: 'Total Rental Fee:',
    btnConfirmBooking: 'CONFIRM BOOKING',
    bookingCodeLabel: 'Booking Code:',
    saveCodeNotice: 'Save this code for on-site check-in at the venue.',
    paymentStatusLabel: 'Payment Status:',
    payTransferNote: '(Pay On-Site / Transfer)',
    bankInstructionsTitle: 'Bank Transfer Instructions:',
    btnCloseReturn: 'CLOSE & RETURN HOME',
    multiBookingTipTitle: 'Tips for Booking Multiple Courts:',
    multiBookingTipDesc: 'Use the exact same Full Name & WhatsApp Number. As long as your previous invoice is UNPAID, new bookings will be merged under 1 Booking Code & Invoice!',
    mergedBookingNotice: 'This new booking has been automatically merged into your Booking Code',
    rentedCourtsList: 'Rented Courts List:',
    qrisPaymentTitle: 'Payment via QRIS',
    qrisScanDesc: 'Scan QRIS using GoPay, OVO, Dana, ShopeePay, Mobile Banking, etc.',
    processing: 'Processing...',
    paymentDeadlineTitle: 'Payment Deadline Information',
    paymentDeadlineTime: 'Late Payment Deadline',
    orderCreatedTime: 'Booking Creation Time',
    latePaymentNotice: 'Overdue / Late Payment',
    paymentLimitHoursLabel: 'Payment Deadline Limit (Hours)',
    paymentLimitHoursSub: 'Set payment deadline duration after booking creation (default: 1 hour)',

    // Check Booking Modal
    checkModalTitle: 'Check Booking Status & Payment',
    checkInputPlaceholder: 'Example: SB-1001 or 081234567890',
    btnCheck: 'CHECK',
    searching: 'Searching...',
    uploadProofTitle: 'Upload Payment Receipt Proof',
    uploadProofSub: 'Upload transfer receipt photo for staff verification.',
    btnUploadProof: 'UPLOAD TRANSFER PROOF',
    uploadingProofMsg: 'Uploading...',
    proofReadyMsg: 'Transfer Proof Ready',
    proofReadySub: 'Click button below to upload',
    orWhatsApp: 'OR PROOF VIA WHATSAPP',
    btnSendWA: 'SEND PAYMENT PROOF TO ADMIN WHATSAPP',
    bankPaymentDetails: 'Venue Payment Bank Account',
    scanQris: 'Scan QRIS',

    // Admin Login Modal
    staffLoginTitle: 'Staff & Venue Operator Login Portal',
    usernameLabel: 'Staff Username *',
    passwordLabel: 'Password *',
    usernamePlaceholder: 'Enter username',
    passwordPlaceholder: 'Enter password',
    btnEnterDashboard: 'ENTER DASHBOARD',
    loggingIn: 'Processing Login...',
    demoAccountsTitle: 'Access Accounts by Role Level:',
    returnToHome: 'Return to Main Landing Page',

    // Manual Staff Booking Modal
    manualBookingTitle: 'Manual Staff Booking (On-site / Phone)',
    manualBookingSuccessTitle: 'Manual Booking Created Successfully!',
    manualBookingSuccessSub: 'Schedule updated in system and visual calendar grid.',
    selectCourtLabel: 'Select Court *',
    custDataManualTitle: 'Customer Details (Walk-in / Phone)',
    custNameLabel: 'Customer Name *',
    btnClose: 'CLOSE',
    btnCancel: 'Cancel',
    btnSaveManualBooking: 'SAVE MANUAL BOOKING',
    savingManualBooking: 'Saving...',

    // Management Dashboard (Admin)
    adminTitle: 'Management Console',
    btnManualStaff: 'MANUAL STAFF BOOKING',
    navDashboard: 'Dashboard',
    navBookings: 'Bookings',
    navCalendar: 'Calendar',
    navCourts: 'Courts',
    navSports: 'Sports',
    navCustomers: 'Customers',
    navPayments: 'Payments',
    navReports: 'Reports',
    navUsers: 'Users',
    navSettings: 'Settings',
    btnViewMainWeb: 'View Main Website',
    statTodayRev: "Today's Revenue",
    statTodayBookings: "Today's Bookings",
    statOccupiedCourts: 'Occupied Courts',
    statOccupancyRate: 'Occupancy Rate',
    tableBookingCode: 'Booking Code',
    tableCustomer: 'Customer',
    tableCourtSport: 'Court & Sport',
    tableDateTime: 'Date & Time',
    tableTotalPrice: 'Total Price',
    tableProof: 'Payment Proof',
    tableStatus: 'Booking Status',
    tableActions: 'Manage Status / Staff Actions',
    btnDeletePermanent: 'Delete Permanent',
    btnSetPaid: 'Set Paid',
    btnViewProof: 'View Proof',

    // Overview Tab
    loadingDashboard: 'Loading dashboard data...',
    upcomingBookingsTitle: 'Upcoming Bookings',
    upcomingBookingsSub: 'List of upcoming reservations',
    statBySport: 'Statistics by Sport',
    sessions: 'Sessions',
    recordedInSystem: 'Recorded in system',
    courtsAvailable: 'Courts Available',
    noUpcomingBookings: 'No upcoming bookings yet',

    // Bookings & Payments Tab
    searchBookingsPlaceholder: 'Search booking code, customer name, or phone...',
    allBookingStatus: 'All Booking Statuses',
    exportExcel: 'EXPORT EXCEL',
    noBookingsFound: 'No booking records found.',
    showingData: 'Showing',
    to: 'to',
    of: 'of',
    bookingsCount: 'booking records',
    exportModalTitle: 'Export Data to Excel',
    exportDateStart: 'Start Date *',
    exportDateEnd: 'End Date *',
    downloadExcelBtn: 'DOWNLOAD EXCEL FILE',
    transferProofModalTitle: 'Payment Transfer Proof',
    verifySetPaidBtn: 'VERIFY & SET PAID',
    deleteProofBtn: 'Delete Proof',
    btnRefreshBookings: 'REFRESH & CHECK OVERDUE',
    autoCancelledMsg: 'overdue booking(s) automatically cancelled.',
    noOverdueMsg: 'No overdue bookings found to cancel.',
    btnPrintReceipt: 'Print Receipt',
    btnPrintReceiptShort: 'Print Receipt',
    receiptTitle: 'PAYMENT RECEIPT',
    receiptSub: 'Official Payment & Court Rental Receipt',
    receiptNo: 'Receipt / Booking No.',
    receiptDate: 'Date Printed',
    receiptCustomerInfo: 'Customer Information',
    receiptBookingDetails: 'Rental Details',
    receiptItem: 'Item / Court',
    receiptSchedule: 'Schedule & Duration',
    receiptPrice: 'Price',
    receiptSubtotal: 'Subtotal',
    receiptTotalPayment: 'TOTAL PAYMENT',
    receiptPaymentMethod: 'Payment Method',
    receiptPaymentStatus: 'Payment Status',
    receiptBookingStatus: 'Booking Status',
    receiptStatusPaid: 'PAID',
    receiptStatusFinished: 'FINISHED',
    receiptThankYou: 'Thank you for booking and playing with us!',
    receiptContactNotice: 'Please keep this receipt as official proof of payment.',
    paymentsFinishedNotice: 'Showing booking payment transactions with status Finished.',
    eposPrinterMode: 'Thermal EPOS Printer (58mm / 80mm POS)',
    standardPrinterMode: 'Standard Document (A4)',
    btnPrintEpos: 'PRINT TO EPOS PRINTER',
    eposLayoutTitle: 'EPOS Thermal Receipt Format',

    // Calendar Tab
    visualCalendarTitle: 'Visual Court Schedule Grid',
    visualCalendarSub: 'Court time slot matrix. Click occupied slot to manage status or delete.',
    hourHeader: 'Time',
    loadingCalendar: 'Loading calendar schedule...',
    manageBookingStatusTitle: 'Manage Staff Booking Status',
    currentStatusLabel: 'Current Status:',
    finishBtn: 'Done',

    // Courts Tab
    courtMgmtTitle: 'Court Directory Management',
    courtMgmtSub: 'Manage courts, hourly rates, photos, and active availability',
    addNewCourtBtn: 'ADD NEW COURT',
    editCourtTitle: 'Edit Court Details',
    addNewCourtTitle: 'Add New Court',
    sportCategoryLabel: 'Sport Category *',
    courtNameLabel: 'Court Name *',
    pricePerHourLabel: 'Hourly Rate (IDR) *',
    imageUrlLabel: 'Court Image URL',
    facilitiesCommaLabel: 'Facilities (Comma separated)',
    courtStatusLabel: 'Court Status',
    activeStatus: 'Active (Available)',
    maintenanceStatus: 'Maintenance (Under Repair)',
    saveCourtBtn: 'Save Court',
    loadingCourts: 'Loading courts list...',

    // Sports Tab
    addSportHeading: 'Add Sport Category',
    sportNameLabel: 'Sport Name *',
    shortDescLabel: 'Short Description',
    addSportBtn: '+ ADD SPORT',
    sportsListHeading: 'Registered Sport Categories',

    // Customers Tab
    customerDirTitle: 'Customer Directory',
    customerDirSub: 'Customer rental history, total bookings, and total spent',
    searchCustomerPlaceholder: 'Search name, phone, or email...',
    noCustomersFound: 'No customer records found.',
    loadingCustomers: 'Loading customer data...',
    totalBookingsLabel: 'Total Booking Sessions',
    totalSpentLabel: 'Total Spent',

    // Reports Tab
    reportsHeading: 'Revenue & Occupancy Reports',
    reportsSub: 'Business performance summary by court and sport branch',
    revenuePerCourt: 'Revenue Report by Court',
    occupancyMetrics: 'Occupancy Rate Metrics',
    todayUsage: 'Today Usage Rate',
    totalActiveCourts: 'Total Active Courts:',
    occupiedCourtsCount: 'Occupied Courts:',
    emptyCourtsCount: 'Available Courts:',

    // Settings Tab
    venuePaymentConfig: 'Venue Payment Configuration',
    paymentSettingsHeading: 'Payment Methods Settings',
    paymentSettingsSub: 'Configure venue bank account details & QRIS image for customer payments.',
    bankInfoHeading: 'Venue Bank Account Information',
    bankInfoSub: 'Bank transfer details displayed on customer booking receipts.',
    bankNameField: 'Bank Name *',
    accountNumberField: 'Account Number *',
    accountHolderField: 'Account Holder Name *',
    waSupportHeading: 'WhatsApp Support / Live Chat Number',
    waSupportSub: 'This WhatsApp number is used for the floating widget on homepage & payment verification.',
    waAdminField: 'Admin WhatsApp Number (Country code 62...) *',
    qrisInfoHeading: 'QRIS Information & Barcode',
    qrisInfoSub: 'Manage venue QRIS barcode image for instant digital payments.',
    merchantNameField: 'Merchant Name / QRIS Description',
    activeQrisImage: 'Active QRIS Barcode',
    changeQrisBtn: 'Change QRIS Image',
    deleteQrisBtn: 'Delete QRIS',
    noQrisTitle: 'No QRIS Image Uploaded',
    noQrisSub: 'Upload your venue QRIS barcode image (PNG, JPG, or WebP format).',
    uploadNewQrisBtn: 'Upload New QRIS Image',
    savePaymentSettingsBtn: 'SAVE PAYMENT SETTINGS',
    savingSettingsMsg: 'Saving...',

    // Users Management Tab
    usersMgmtTitle: 'User & Branch Staff Management',
    usersMgmtSub: 'Manage Super Admin and Outlet Operator accounts with isolated multi-branch access permissions',
    exportUsersTitle: 'Export staff data to Excel file (.csv)',
    addStaffBtn: 'Add Staff / User',
    totalUsersLabel: 'Total Users',
    superAdminLabel: 'Super Admin',
    operatorOutletLabel: 'Outlet Operator',
    activeAccountsLabel: 'Active Accounts',
    searchUsersPlaceholder: 'Search staff name / username...',
    allRoles: 'All Roles',
    roleSuperAdmin: 'Super Admin',
    roleOperatorOutlet: 'Outlet Operator',
    allBranches: 'All Branches',
    registeredStaff: 'Registered Staff',
    colStaffName: 'Staff Name & Username',
    colRole: 'Role / Level',
    colAssignedOutlet: 'Assigned Outlet Branch',
    colStatus: 'Status',
    colCreatedAt: 'Date Created',
    colActions: 'Actions',
    loadingUsers: 'Loading user data...',
    noUsersFound: 'No users match the filter.',
    allBranchesFullAccess: 'All Branches (Full Access)',
    unassignedRole: 'Unassigned',
    statusActive: 'Active',
    statusInactive: 'Inactive',
    editUserTooltip: 'Edit User',
    deleteUserTooltip: 'Delete User',
    editStaffTitle: 'Edit Staff Account',
    addStaffTitle: 'Add New Staff Account',
    labelFullNameStaff: 'Full Staff Name:',
    phFullNameStaff: 'e.g., Rian Anggara',
    labelUsernameLogin: 'Login Username:',
    phUsernameLogin: 'e.g., staff_cilandak',
    helpUsername: 'Lowercase letters, numbers, and underscores only',
    labelNewPassword: 'New Password (Leave blank if unchanged):',
    labelAccountPassword: 'Account Password:',
    phKeepPassword: '•••••••• (Keep unchanged)',
    phMin6Chars: 'Minimum 6 characters',
    labelRoleAccess: 'Role / Access Level:',
    optOperator: 'Outlet Operator (Isolated per Branch)',
    optAdmin: 'Super Admin (Full Access All Branches)',
    labelAssignOutlet: 'Assign to Outlet Branch:',
    optSelectOutlet: '-- Select Outlet Branch --',
    helpOutletAssign: 'This staff can only manage court data, schedules, bookings, and payments at the assigned outlet branch.',
    labelAccountStatus: 'Account Status:',
    optActive: 'Active',
    optInactive: 'Inactive (Block Login)',
    btnSaveChanges: 'Save Changes',
    btnCreateUser: 'Create User',
    savingMsg: 'Saving...',
    errPasswordRequired: 'Password is required for new users',
    errOperatorOutletRequired: 'Users with Operator role must be assigned to an outlet',
    errSaveUser: 'Failed to save user',
    alertCannotDeleteSuperAdmin: 'Primary Super Admin cannot be deleted!',
    errDeleteUser: 'Failed to delete user',
    excelUserId: 'User ID',
    excelFullName: 'Full Name',
    excelUsername: 'Username',
    excelRole: 'Role / Level',
    excelOutletName: 'Assigned Outlet Branch',
    excelStatus: 'Account Status',
    excelCreatedAt: 'Date Created'
  }
};
