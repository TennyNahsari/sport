...
export default function OverviewTab({ onOpenManualBooking }) {
  ...
  if (loading) {
    return <div className="p-8 text-center text-slate-500 font-bold">Memuat data dashboard...</div>;
  }
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Top Stats 4 Cards Grid (PRD Section 17) */}
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5">
        ...
        <div className="bg-white p-5 rounded-card border border-slate-200 shadow-sm flex items-center justify-between">
          <div>
            <span className="text-xs font-bold text-slate-500 uppercase">Revenue Hari Ini</span>
            ...
            <span className="inline-flex items-center gap-1 text-[11px] font-bold text-sportgreen mt-1">
              <TrendingUp className="w-3.5 h-3.5" /> +12% dibanding kemarin
            </span>
          </div>
        ...
        <div className="bg-white p-5 rounded-card border border-slate-200 shadow-sm flex items-center justify-between">
          <div>
            <span className="text-xs font-bold text-slate-500 uppercase">Booking Hari Ini</span>
            <p className="text-2xl font-extrabold text-navy mt-1">{data.todayBookings} Sesi</p>
            <span className="text-[11px] font-bold text-slate-400 mt-1 block">Tercatat di sistem</span>
          </div>
        ...
        <div className="bg-white p-5 rounded-card border border-slate-200 shadow-sm flex items-center justify-between">
          <div>
            <span className="text-xs font-bold text-slate-500 uppercase">Lapangan Terpakai</span>
            ...
            <span className="text-[11px] font-bold text-orange mt-1 block">
              {data.availableCourts} Lapangan Tersedia
            </span>
          </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
        ...
        <div className="lg:col-span-8 bg-white rounded-card border border-slate-200 shadow-sm p-6">
          <div className="flex items-center justify-between mb-4">
            <div>
              <h3 className="font-extrabold text-navy text-lg">Upcoming Bookings</h3>
              <p className="text-xs text-slate-500">Daftar reservasi mendatang</p>
            </div>
            <button
            ...
            <table className="w-full text-left text-xs">
              ...
              <tbody className="divide-y divide-slate-100 font-semibold text-navy">
                {data.upcomingBookings.length === 0 ? (
                  <tr>
                    <td colSpan={5} className="p-6 text-center text-slate-400">Belum ada booking mendatang</td>
                  </tr>
                ) : (
        ...
        <div className="lg:col-span-4 bg-white rounded-card border border-slate-200 shadow-sm p-6">
          <h3 className="font-extrabold text-navy text-lg mb-4">Statistik Per Olahraga</h3>
            ...
            {data.bookingsBySport.map((sport, idx) => (
              <div key={idx} className="space-y-1">
                <div className="flex justify-between text-xs font-bold text-navy">
                  ...
                  <span className="text-primary">{sport.booking_count} Booking</span>
                </div>
